...
SELECT nombre, precio, (precio * 1.05) AS dolares FROM producto;

#5.Llista el nom dels productes, el preu en euros i el preu en dòlars estatunidencs (USD). Utilitza els següents àlies per a les columnes: nom de producto, euros, dòlars:
SELECT nombre AS 'nom de producto', precio AS euros, (precio * 1.05) AS dolares FROM producto;
...
SELECT p.nombre FROM producto p WHERE p.codigo_fabricante = (SELECT codigo FROM fabricante WHERE nombre = 'Lenovo') ORDER BY p.precio DESC LIMIT 1;

#39.Llista el nom del producte més barat del fabricant Hewlett-Packard:
SELECT p.nombre FROM producto p WHERE p.codigo_fabricante = (SELECT codigo FROM fabricante WHERE nombre = 'Hewlett-Packard') ORDER BY p.precio ASC LIMIT 1;

#40.Retorna tots els productes de la base de dades que tenen un preu major o igual al producte més car del fabricant Lenovo:
SELECT p.nombre, p.precio FROM producto p WHERE p.precio >= (SELECT MAX(precio) FROM producto WHERE codigo_fabricante = (SELECT codigo FROM fabricante WHERE nombre = 'Lenovo'));

#41.Llista tots els productes del fabricant Asus que tenen un preu superior al preu mitjà de tots els seus productes:
SELECT p.nombre, p.precio FROM producto p WHERE p.codigo_fabricante = (SELECT codigo FROM fabricante WHERE nombre = 'Asus') AND p.precio > (SELECT AVG(precio) FROM producto WHERE codigo_fabricante = (SELECT codigo FROM fabricante WHERE nombre = 'Asus'));